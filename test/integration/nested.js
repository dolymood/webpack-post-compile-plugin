...
describe('nested case', function () {
  it('webpack options should be correct', function (done) {
    webpack(webpackConfig, function (err, stats) {
      ...
      expect(rule.include.length).to.equal(4)
      ...
      expect(rule.include[3]).to.equal(path.resolve(__dirname, '../cases/nested/node_modules/b'))
      var ret = require('../cases/nested/app.js')
      expect(ret.a).to.equal('nested a')
      expect(ret.b).to.equal('nested b, nested inner a')
